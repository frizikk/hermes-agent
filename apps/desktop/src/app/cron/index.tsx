...
export function CronJobRuns({ c, jobId, profile }: { c: Translations['cron']; jobId: string; profile?: string }) {
  ...
  useEffect(() => {
    ...
    if (runs.some(run => run.id === focusTarget.outputId)) {
      ...
      // The job detail stays open (selected by the sidebar-focus effect above);
      // only the requested run is gone, e.g. pruned by output retention.
      notify({ kind: 'info', message: c.outputUnavailable })
    }

    setCronFocusJobId(null)
  }, [c, focusTarget, jobId, openOutput, profile, runs])
