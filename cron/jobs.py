...
from hermes_time import get_timezone as _hermes_get_timezone
...
_CRON_OUTPUT_ID_FORMAT = "%Y-%m-%d_%H-%M-%S"


def _cron_output_id_to_epoch(output_id: str) -> float:
    """Convert a validated cron output id back to epoch seconds.

    ``save_job_output`` names each file from ``_hermes_now()`` wall-clock
    fields, so parse those fields back with the same timezone rather than
    trusting the file's mtime — mtime can drift from the id (copies,
    backups, restores, clock changes) while the id itself cannot lie about
    when the run actually happened.
    """
    naive = datetime.strptime(output_id, _CRON_OUTPUT_ID_FORMAT)
    tz = _hermes_get_timezone()
                    ...
                    outputs.append(
                        {
                            ...
                            "created_at": _cron_output_id_to_epoch(output_id),
                        }
                    )
    except FileNotFoundError:
        return []
...
def get_job_output(job_id: str, output_id: str) -> Dict[str, Any]:
    ...
    with _open_job_output_directory(job_id) as (job_fd, job_dir):
        ...
        if job_fd is None:
            # Portability fallback for platforms without dir_fd support (see
            # _open_job_output_directory): the parent directory chain was
            # already validated component-by-component with lstat+resolve
            # above, but — unlike the dir_fd path — it isn't pinned by open
            # file descriptors, so a parent could still be swapped between
            # validation and this open. O_NOFOLLOW at least keeps the final
            # open from following a symlink swapped in for the leaf file
            # itself; the fstat identity check below still catches a swapped
            # regular file.
