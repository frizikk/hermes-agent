...
def test_cron_run_outputs_are_read_from_the_owning_profile(
    ...
    # created_at must be derived from the filename id, not mtime. Deliberately
    # drift the file's mtime away from what its name encodes so a regression
    # back to st_mtime would be caught here.
    expected_created_at = cron_jobs._cron_output_id_to_epoch("2026-08-11_09-00-00")
    drifted_mtime = expected_created_at + 3600
    os.utime(newer, (drifted_mtime, drifted_mtime))
    assert newer.stat().st_mtime != expected_created_at
...
def test_cron_run_output_fallback_requests_o_nofollow_on_final_open(
    isolated_profiles,
    monkeypatch,
):
    """The no-dir_fd portability fallback must still ask the final open to
    refuse a symlink swapped in for the leaf file itself, even though the
    validated parent directory chain is not pinned by open descriptors in
    that fallback (see _open_job_output_directory)."""
    from cron import jobs as cron_jobs
    from hermes_cli import web_server

    if not getattr(cron_jobs.os, "O_NOFOLLOW", 0):
        pytest.skip("platform has no O_NOFOLLOW to request")
    ...
    def recording_open(path, flags, *args, **kwargs):
        if Path(path).name == planted.name:
            captured_flags.append(flags)
        return real_open(path, flags, *args, **kwargs)

    monkeypatch.setattr(cron_jobs.os, "open", recording_open)

    detail = web_server._get_cron_job_output_sync(
        job["id"], planted.stem, profile="worker_alpha"
    )

    assert detail["content"] == "safe output"
    assert captured_flags, "expected the fallback path to open the leaf file directly"
    assert all(flags & cron_jobs.os.O_NOFOLLOW for flags in captured_flags)
